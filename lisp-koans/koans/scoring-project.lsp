...
(defun score (dice)
  (let ((counts
	 (loop for i in dice
	       when ((lambda (z) (= z 1)) i) count i into ones
	       when ((lambda (z) (= z 2)) i) count i into twos
	       when ((lambda (z) (= z 3)) i) count i into threes
	       when ((lambda (z) (= z 4)) i) count i into fours
	       when ((lambda (z) (= z 5)) i) count i into fives
	       when ((lambda (z) (= z 6)) i) count i into sixes
	       finally (return (list ones twos threes fours fives sixes)))))
    (destructuring-bind (ones twos threes fours fives sixes) counts
      (let ((result 0))
	(incf result (cond ((= ones 5) 1200)
			   ((= ones 4) 1100)
			   ((= ones 3) 1000)
			   ((= ones 2) 200)
			   ((= ones 1) 100)
			   (t 0)))
	(incf result (cond ((= fives 5) 600)
			   ((= fives 4) 550)
			   ((= fives 3) 500)
			   ((= fives 2) 100)
			   ((= fives 1) 50)
			   (t 0)))
	(incf result (cond ((>= twos 3) 200)
			   ((>= threes 3) 300)
			   ((>= fours 3) 400)
			   ((>= sixes 3) 600)
			   (t 0)))
